...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  // SMS Logs & Broadcast Audit Stream
  ...
  const handleUpdateStatus = async (newStatus: string) => {
    ...
    if (newStatus === 'REJECTED') {
      return handleForceReject();
    }
    if (newStatus === 'VERIFIED') {
      return handleForceVerify();
    }
  ...
  const handleDispatchSectorDrill = async () => {
    ...
    try {
      const res = await dispatchRealSMS('Kohima (NH-29)', 'Evacuation protocol active. Avoid mountain corridor.');
      if (typeof window !== 'undefined') {
        alert(`🚨 LIVE SMS DELIVERED!\n\nTarget: +91 ${res.phone}\nGateway: Fast2SMS Quick Route (Active)\nCheck your physical phone now.`);
      }
      setDrillSuccess(`🚨 LIVE SMS DELIVERED! Target: +91 ${res.phone}`);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 flex flex-wrap items-center justify-between gap-4 shadow-xs">
        ...
        <div className="flex items-center gap-1 bg-slate-100/80 p-1 rounded-lg border border-slate-200/80 flex-wrap">
          ...
          <button
            id="btn-admin-tab-sms-logs"
            onClick={() => setActiveTab('sms_logs')}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs sm:text-sm font-medium transition cursor-pointer ${
              activeTab === 'sms_logs' ? 'bg-indigo-600 text-white shadow-xs font-semibold' : 'text-slate-600 hover:bg-slate-200/60 hover:text-slate-900'
            }`}
          >
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4 shadow-xs">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            ...
            >
              ...
              <span>Trigger Emergency Alert (CAP)</span>
            </button>
          </div>
      ...
      {/* TAB: SMS BROADCAST & CELL DISPATCH LOGS */}
      {activeTab === 'sms_logs' && (
        <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-6 shadow-sm">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h2 className="text-base sm:text-lg font-bold text-slate-900 flex items-center gap-2">
                    <span>Emergency SMS Cell Broadcast &amp; Dispatch Stream</span>
                  </h2>
                  <p className="text-xs text-slate-500">
                    Audit trail of all automated and manual SMS alerts dispatched to citizen subscribers and local responders.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              >
                <RotateCcw className={`w-3.5 h-3.5 ${smsLogsLoading ? 'animate-spin' : ''}`} />
                <span>{smsLogsLoading ? 'Refreshing...' : 'Refresh Feed'}</span>
              </button>
              <button
                id="btn-admin-send-drill-sms"
                onClick={handleAdminSendDrillSms}
                disabled={isSendingDrillSms}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded-lg text-xs font-bold shadow-xs transition cursor-pointer disabled:opacity-50"
              >
                <Send className="w-3.5 h-3.5" />
                <span>{isSendingDrillSms ? 'Transmitting Drill...' : 'Dispatch Sector Drill SMS'}</span>
              </button>
            </div>
          </div>

          {/* Drill success message */}
          {drillSuccess && (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="bg-slate-50 border border-slate-200/80 rounded-xl p-4 text-center">
              <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Total Dispatches</div>
              <div className="text-2xl font-bold font-mono text-slate-900 mt-0.5">{smsLogs.length}</div>
              <div className="text-[10px] text-slate-400 mt-0.5">Automated &amp; CAP events</div>
            </div>

            <div className="bg-emerald-50/60 border border-emerald-200/80 rounded-xl p-4 text-center">
              <div className="text-[10px] font-bold text-emerald-700 uppercase tracking-wider">Incident Verified</div>
              <div className="text-2xl font-bold font-mono text-emerald-900 mt-0.5">
                {smsLogs.filter((l) => l.trigger_type === 'INCIDENT_VERIFIED').length}
              </div>
              <div className="text-[10px] text-emerald-600 mt-0.5">Field hazard verification</div>
            </div>
            ...
            <div className="bg-indigo-50/60 border border-indigo-200/80 rounded-xl p-4 text-center">
              <div className="text-[10px] font-bold text-indigo-700 uppercase tracking-wider">Gateway Status</div>
              <div className="text-sm font-bold font-mono text-indigo-900 mt-1">100% ACK</div>
              <div className="text-[10px] text-indigo-600 mt-0.5">National Cell Gateway</div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-slate-100">
            <div className="flex items-center gap-2 text-xs">
              <Filter className="w-3.5 h-3.5 text-slate-400" />
              <span className="font-semibold text-slate-600">Filter by Sector:</span>
              <select
                id="select-admin-sms-filter"
                value={smsSectorFilter}
                onChange={(e) => setSmsSectorFilter(e.target.value)}
                className="bg-white border border-slate-300 rounded-lg px-2.5 py-1 text-xs text-slate-700 focus:outline-none focus:border-indigo-500 font-semibold"
              >
                <option value="ALL">All Sectors ({smsLogs.length})</option>
            ...
            <span className="text-[11px] font-mono text-slate-400">
              Protocol: CAP-v1.2 &bull; GSM 03.40 SMPP
            </span>
          </div>

          {/* SMS Logs List */}
          {smsLogs.length === 0 ? (
            <div className="py-16 text-center bg-slate-50 border border-slate-200 rounded-xl text-slate-500 text-sm space-y-1">
              <CheckCircle className="w-8 h-8 text-emerald-500 mx-auto mb-1" />
              <div className="font-bold text-slate-800">No Emergency SMS Broadcasts Dispatched Yet</div>
              <p className="text-xs text-slate-400">
                Verifying a citizen field incident or triggering a CAP emergency alert will automatically dispatch and log SMS transmissions here.
              </p>
            </div>
          ) : (
            <div className="space-y-3 max-h-[600px] overflow-y-auto pr-1">
              {smsLogs
                .filter((l) =>
                  smsSectorFilter === 'ALL'
                    ? true
                    : l.recipient_sector?.toLowerCase().includes(smsSectorFilter.toLowerCase().slice(0, 5)) ||
                      smsSectorFilter.toLowerCase().includes((l.recipient_sector || '').toLowerCase().slice(0, 5))
                )
                .map((log) => (
                  <div
                    key={log.id}
                    className="p-4 rounded-xl bg-slate-50 border border-slate-200 hover:border-slate-300 text-xs space-y-2.5 transition shadow-2xs"
                  >
