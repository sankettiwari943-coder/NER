...
export function calculateLandslideRisk(
  ...
  const contributingFactors: FactorContribution[] = [
    ...
    {
      factor: 'Sentinel-1 SAR Radar Deformation (InSAR)',
      category: 'SATELLITE_SAR',
      weight: weightSar,
      rawScore: Math.round(sarNormalizedScore),
      weightedScore: Number(weightedSar.toFixed(1)),
      ...
      explanation: `Spaceborne C-Band SAR ground deformation (+${satelliteData.sentinel1Sar.pointsContribution} pts). Note: Radar penetrates monsoonal cloud cover.`,
    },
    {
      factor: 'Sentinel-2 Optical NDVI & Bare Soil Index',
      category: 'SATELLITE_OPTICAL',
      weight: weightOptical,
      rawScore: Math.round(opticalNormalizedScore),
      weightedScore: Number(weightedOptical.toFixed(1)),
      ...
      explanation: `Multispectral optical vegetation loss and fresh scarp bare soil exposure contribute +${satelliteData.sentinel2Optical.pointsContribution} pts.`,
    },
    {
